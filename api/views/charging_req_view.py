...
from api.models import ChargingRequest, User, ParkingSpot
...
class ChargingRequestViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"error": "Parking spot does not exist"}, status=status.HTTP_404_NOT_FOUND)
        ...
        mqtt_publish.single(f"mwbot/dispatch/1", message, hostname=MQTT_BROKER, port=MQTT_PORT)

        return Response({"message": f"Charging requested for {car_model} at spot {spot.spot_id} for {duration} minutes by {user.name}"}, status=status.HTTP_201_CREATED)
